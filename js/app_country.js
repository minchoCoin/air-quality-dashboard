...
function makecsv(response){
    ...
    let content = "time,city,country,temp_c,humidity,condition,wind_kph,wind_dir,pressure_mb,feelslike_c,uv\n";
    for (var i = 0; i < 3; ++i) {
        let forecastinfo = response.forecast.forecastday[i].hour
        forecastinfo.forEach((item, index) => {
            let newrow = `${item.time},${response.location.name},${response.location.country},${item.temp_c},${item.humidity},${item.condition.text},${item.wind_kph},${item.wind_dir},${item.pressure_mb},${item.feelslike_c},${item.uv}\n`
    ...
    link.download = `${response.location.name}.csv`;
